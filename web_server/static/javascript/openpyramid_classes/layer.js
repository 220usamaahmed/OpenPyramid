...
class Layer {
	...
	display(dx, dy) {
		...
		let x = (dx < 0) ? (dx * this.rX) % this.tileSize : dx; // This might not be dx

		while (tileXIndex < this.tileCountX && x < this.canvasWidth) {

			let tileYIndex = (dy < 0) ? floor(-dy * this.rY / this.tileSize) : 0; 
			let y = (dy < 0) ? (dy * this.rY) % this.tileSize : dy; // This might not be dx
	...
	displayTile(x, y, tileXIndex, tileYIndex) {
		let url = "/slide/" + this.level + "/" + tileXIndex + "_" + tileYIndex + ".png";
